...
EDGE2_lists <- function(EDGE2_scores, ED2_values, IUCN_status){
  ...
  for (s in 1:nrow(df_lists)) {
    ...
    df_lists$ED2[s] <- median(ED2_values[df_lists$scientific_name[s],], na.rm=T) # median
    df_lists$ED2_sd[s] <- sd(ED2_values[df_lists$scientific_name[s],], na.rm=T) # sd
