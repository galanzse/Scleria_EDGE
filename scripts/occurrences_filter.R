...
occ_scleria <- read_excel("data/occ_scleria_taxcorrected.xlsx", sheet="occ_scleria") %>%
  dplyr::select(-source, -id) # import occurrences
length(unique(occ_scleria$scientificName))

# 
colnames(occ_scleria)[colnames(occ_scleria)%in%c("scientificName","decimalLongitude","decimalLatitude")] <- c('species','x','y') # correct variable names
...
for (s in unique(occ_scleria$species)) {
  ...
  if (nrow(ss_df) > 5) {
    ss_df <- ss_df %>% filter(MAT > boxplot.stats(ss_df$MAT, coef=2)$stats[1] &
                              MAT < boxplot.stats(ss_df$MAT, coef=2)$stats[5] &
                              AP > boxplot.stats(ss_df$AP, coef=2)$stats[1] &
                              AP < boxplot.stats(ss_df$AP, coef=2)$stats[5] )
  }
